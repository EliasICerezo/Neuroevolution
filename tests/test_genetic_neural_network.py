...
class TestGeneticNN:
  ...
  def test_basic_construction_raises_error_when_pop_size_is_over_maximum(self):
    pop_size = 500
  ...
  def test_basic_construction_raises_error_with_0_max_individuals(self):
    pop_size = 0
    with pytest.raises(AttributeError):
      _ = GeneticNeuralNetwork([3,1],1,4,None,pop_size)
  
  def test_evolved_feed_foeward_returns_an_array_with_pop_size(self):
    pop_size = 25
    nnet = GeneticNeuralNetwork([3,1],1,3,None,pop_size)
